...
const classStartDate = new Date(Date.UTC(2012, 6, 1));

function yearsOfBrotherhood(today = new Date()) {
  let years = today.getUTCFullYear() - classStartDate.getUTCFullYear();
  const anniversaryThisYear = new Date(Date.UTC(today.getUTCFullYear(), 6, 1));
...
export default async function Home() {
  ...
  const [event, announcements, legacyBrothers, prefects, countryGroups] = await Promise.all([
    ...
    MemberProfile.aggregate([
      ...
      { $unwind: "$member" },
      { $match: { "member.status": "verified", "member.roles": { $nin: ["admin", "super_admin"] } } },
      { $group: { _id: { $toLower: { $trim: { input: "$country" } } } } },
      { $match: { _id: { $ne: "" } } },
    ]).exec() as Promise<any[]>,
  ]);
  const brotherhoodYears = yearsOfBrotherhood();
  const countriesRepresented = countryGroups.length;
  const updates = [
  ...
  return <>
    <SiteHeader />
    <main>
      <section className="hero"><div className="container"><div className="hero-content"><div className="hero-kicker"><i /> Mount Saint Gabriel&apos;s · Class of 2012</div><h1>From the Mount.<br /><em>For a lifetime.</em></h1><p>More than classmates. We are a brotherhood forged by shared memories, strengthened by every journey, and united by a legacy that still calls us home.</p><div className="hero-actions"><Link className="btn btn-primary" href="/register">Join the community <ArrowRight size={17} /></Link><Link className="btn btn-outline" href="/members">Explore our directory</Link></div></div></div><div className="hero-foot"><div className="container hero-stats"><div className="stat"><b>180+</b><span>Classmates</span></div><div className="stat"><b>{brotherhoodYears}</b><span>Years of brotherhood</span></div><div className="stat"><b>{countriesRepresented}</b><span>Countries represented</span></div><div className="stat"><b>One</b><span>Unbreakable bond</span></div></div></div></section>
