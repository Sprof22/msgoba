...
export default function PrefectsAdminPage() {
    const [items, setItems] = useState<Prefect[]>([]);
    ...
    async function load() {
        const response = await fetch("/api/admin/prefects");
        const data = await response.json();
        setItems(data.prefects || []);
        setLoading(false);
    }
    ...
    function openEditor(item?: Prefect) {
        setError("");
        setCover(null);
        if (!item) {
    ...
    async function onSubmit(event: FormEvent<HTMLFormElement>) {
        ...
        const url = editing._id
            ? `/api/admin/prefects/${editing._id}`
            : "/api/admin/prefects";
    ...
    async function remove(id: string) {
        if (!confirm("Delete this prefect?")) return;
        await fetch(`/api/admin/prefects/${id}`, { method: "DELETE" });
        await load();
    }
            ...
            <div className="media-container wide">
                ...
                <div className="media-heading">
                    <div>
                        ...
                        <h1>Leadership prefects section</h1>
                        <p>
                            Create and maintain the members shown in the "Meet the prefects
                            behind the leadership" section on the homepage.
                        </p>
                    </div>
                    ...
                    <div className="modal-backdrop">
                        <form className="content-modal card" onSubmit={onSubmit}>
                            ...
                            <div className="edit-form-grid">
                                ...
                                <Field label="Post / title" name="post" value={editing.post} />
                                <Field
                                    label="Display order"
                                    name="order"
                                    type="number"
